...
SYSCALL_DEFINE3(my_sys_call,int32_t *, array, int32_t, array_length,int32_t *,dest)
{
	...
	if(array == NULL)
	{
		printk("NULL pointer passed as argument for source array\n");
		return EINVAL;
	}
	if(dest == NULL)
	{
		printk("NULL pointer is passed for destination array\n");
		return EINVAL;
	}

	//Checks if the array length is valid
	if(array_length <= 0)
	{
		printk("Array length is invalid\n");
		return EINVAL;
	}

	// Creates a dynamic array for copying the source to a buffer in kernel area
	int32_t *buf = (int32_t *)kmalloc(array_length * sizeof(int32_t), GFP_KERNEL);
	if(buf == NULL)
	{
		printk("kmalloc failed for buf\n");
		return ENOMEM;
	}
	
	// copies from user area to kernel area
	if((copy_from_user(buf, array, (array_length * sizeof(int32_t))) == 0))
	{
	...
	{
		printk("Error on copy_from_user function\n");
		return EFAULT;
	}
